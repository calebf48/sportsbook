...
for market in markets:
    ...
    if r.status_code == 200:
        ...
        if market == 'h2h':
            with open('static/data/response_ncaafh2h.json', 'w') as file:
                json.dump(json_data, file, indent=4)
            print('NCAAF H2H data saved successfully\n')
        
        elif market == 'spreads':
            with open('static/data/response_ncaafspread.json', 'w') as file:
                json.dump(json_data, file, indent=4)
            print('NCAAF Spread data saved successfully\n')

        elif market =='totals':
            with open('static/data/response_ncaaftotal.json', 'w') as file:
                json.dump(json_data,file, indent=4)
